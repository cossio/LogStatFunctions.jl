...
description: Release a new version of LogStatFunctions.jl and register it in the Julia General registry, suggesting a ColPrac-compliant version number while leaving the final version decision to the user. Use when asked to release, register, tag, or publish a new package version.
---

# Releasing a new version

During development the version in Project.toml carries a `-DEV` suffix (e.g. `2.2.0-DEV`), and changes accumulate under an `## Unreleased` section in CHANGELOG.md.
...
This package is registered in https://github.com/cossio/CossioJuliaRegistry (versions 1.0.0–2.1.0) and is not yet in the General registry. The first General registration goes through the same Registrator comment, but AutoMerge treats it as a **new package**: the initial version number is unconstrained (the standard-initial-version guideline is deactivated), while the name-similarity distance check against `LogExpFunctions` may need the `Override AutoMerge: name similarity is okay` label from a registry maintainer. Whatever version lands first in General anchors the sequential-version check for every later release, and versions below it can no longer be registered there.

## Procedure

1. **Release commit.** After the user explicitly chooses `X.Y.Z`, make a single commit titled `vX.Y.Z` that drops the `-DEV` suffix from `version` in Project.toml and renames the `## Unreleased` CHANGELOG section to `## X.Y.Z`. Land it on `main` (directly, or via a PR).
...
2. **Trigger Registrator on the release commit.** Push the release commit and identify its exact SHA (the merge commit if the release landed via PR). Registration is triggered by posting this comment directly on that commit:
   ...
   - **Post it yourself (preferred).** Run:
     ...
     ```bash
     gh api repos/cossio/LogStatFunctions.jl/commits/<sha>/comments -f body="$(cat <<'EOF'
     @JuliaRegistrator register
